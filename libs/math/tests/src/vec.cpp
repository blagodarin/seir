...
TEST_CASE("Vec3")
{
	...
	SUBCASE("operator Vec2(Vec3)")
	{
		const Vec3 v{ 1, 2, 3 };
		CHECK(seir::Vec2{ v }.x == 1);
		CHECK(seir::Vec2{ v }.y == 2);
	}
}
...
TEST_CASE("Vec4")
{
	...
	SUBCASE("operator Vec3(Vec4)")
	{
		const Vec4 v{ 1, 2, 3, 4 };
		CHECK(seir::Vec3{ v }.x == 1);
		CHECK(seir::Vec3{ v }.y == 2);
		CHECK(seir::Vec3{ v }.z == 3);
	}
}

TEST_CASE("crossProduct")
{
	constexpr seir::Vec3 x{ 1, 0, 0 };
	constexpr seir::Vec3 y{ 0, 1, 0 };
	constexpr seir::Vec3 z{ 0, 0, 1 };
